...
export default function profile(props: any) {
  ...
  const setProfileInfo = async (url: string, name: string) => {
    const { web3FromSource } = await import('@polkadot/extension-dapp');
    const contract = new ContractPromise(api, abi, contractAddress);
    const performingAccount = actingAccount;
    const injector = await web3FromSource(performingAccount.meta.source);
  ...
  const getIndividualPost = async () => {
    ...
    if (output !== undefined && output !== null) {
      setIndividualPostList(output.toHuman() == null ? [] : output.toHuman());
    }
  };
  ...
  const sendMessage = async () => {
    ...
    const add_likes = await contract.tx.sendMessage(
      ...
      'Hey bro! Why do not we go to gym tomorrow?',
      1,
      actingAccount?.address,
      Date().toLocaleLowerCase().toString()
    );
    if (injector !== undefined) {
  ...
  const follow = async () => {
    const { web3FromSource } = await import('@polkadot/extension-dapp');
    const contract = new ContractPromise(api, abi, contractAddress);
    const performingAccount = accounts[1];
    const injector = await web3FromSource(performingAccount.meta.source);
  ...
  const getIdList = () => {
    ...
    for (var i = 0; i < accounts.length; i++) {
      idOptions.push(<option value={i}>{accounts[i].address}</option>);
    }
    return idOptions;
  };
  ...
  const getPostList = () => {
    ...
    for (var i = 0; i < individualPostList.length; i++) {
      postList.push(
        <Post
          name={individualPostList[i].name}
          time={individualPostList[i].createdTime}
          description={individualPostList[i].description}
          num_of_likes={individualPostList[i].numOfLikes}
          user_img_url={imgUrl}
          post_img_url={individualPostList[i].imgUrl}
          follow={follow}
        />
      );
    }
  ...
  useEffect(() => {
    setup();
    console.log(`isCreatedProfile: ${isCreatedProfile}`);
    ...
    <div className="flex justify-center items-center bg-gray-200 w-screen h-screen relative">
      <main className="items-center h-screen w-1/3 flex bg-white flex-col">
        ...
        <ProfileSubTopBar
          ...
          is_open_modal={setShowSettingModal}
          setActingAccount={setActingAccount}
          idList={accounts}
