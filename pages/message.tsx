...
export default function message() {
  ...
  const getProfile = async (userId: any) => {
    ...
    if (output !== undefined && output !== null) {
      ...
      setProfile(output.toHuman());
    }
  };

  // for get lastmessage
  const getProfileMock = async (userId: any) => {
    const contract = new ContractPromise(api, abi, contractAddress);
    const { gasConsumed, result, output } = await contract.query.getProfileInfo(
      '',
      {
  ...
  const sendMessage = async (message: string, id: number, userId: any) => {
    const { web3FromSource } = await import('@polkadot/extension-dapp');
    const contract = new ContractPromise(api, abi, contractAddress);
    const performingAccount = actingAccount;
    const injector = await web3FromSource(performingAccount.meta.source);
  ...
  const createMessageMemberListReal = async () => {
    let memberList: Array<any> = new Array();
    for (var i = 0; i < friendList.length; i++) {
      let friendProfile = await getProfileMock(friendList[i]);
      let idList = profile?.messageListIdList;
      let lastMessage: string;
      let individualMessageList = await getMessageList(idList[i]);
      console.log(`messageList: ${individualMessageList}`);
      if (idList !== null) {
        lastMessage = await getLastMessage(idList[i]);
      }
      let memberListFactor = (
        <MessageMember
          ...
          messageList={individualMessageList}
    ...
    <div className="flex justify-center items-center bg-gray-200 w-screen h-screen relative">
      <main className="items-center h-screen w-1/3 flex bg-white flex-col">
        <TopBar
          ...
          setIsGetProfile={setIsGetProfile}
        />
